package repository

import (
	"carrpigeo/internal/domain"
	"context"
	"database/sql"
	"errors"
	"fmt"

	"github.com/jmoiron/sqlx"
)

var (
	ErrNoTemplate = errors.New("no html template in repository")
)

type HTMLTemplateRepository interface {
	// GetByID returns html template by id.
	// Returns [ErrNoTemplate] if html template not found.
	GetByID(ctx context.Context, id string) (*domain.HTMLTemplate, error)
	// GetMetadataByID returns html template metadata by id.
	// Returns [ErrNoTemplate] if html template not found.
	GetMetadataByID(ctx context.Context, id string) (*domain.HTMLTemplateMetadata, error)
	// GetMetadataByName returns html template metadata by name.
	// Returns [ErrNoTemplate] if html template not found.
	GetMetadataByName(ctx context.Context, name string) (*domain.HTMLTemplateMetadata, error)
	// List returns list of html template metadata with pagination.
	List(ctx context.Context, limit, offset int) ([]domain.HTMLTemplateMetadata, error)
	// Create creates template in db.
	Create(ctx context.Context, template *domain.HTMLTemplate) error
	// Update updates template in db.
	Update(ctx context.Context, template *domain.HTMLTemplate) error
	// Delete deletes template from db.
	Delete(ctx context.Context, id string) error
}

type htmlTemplateRepository struct {
	db *sqlx.DB
}

func NewHTMLTemplateRepository(db *sqlx.DB) HTMLTemplateRepository {
	return &htmlTemplateRepository{
		db: db,
	}
}

func (r *htmlTemplateRepository) GetByID(ctx context.Context, id string) (*domain.HTMLTemplate, error) {
	op := "HTMLTemplateRepository.GetByID"
	var tmpl domain.HTMLTemplate
	query := `
	SELECT id, name, content, created_at, updated_at
	FROM html_templates
	WHERE id = $1
	`
	err := r.db.GetContext(ctx, &tmpl, query, id)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, fmt.Errorf("%s: %w", op, ErrNoTemplate)
		}
		return nil, fmt.Errorf("%s: %w", op, err)
	}
	return &tmpl, nil
}

func (r *htmlTemplateRepository) GetMetadataByID(ctx context.Context, id string) (*domain.HTMLTemplateMetadata, error) {
	op := "HTMLTemplateRepository.GetMetadataByID"
	var meta domain.HTMLTemplateMetadata

	query := `
        SELECT id, name, created_at, updated_at
        FROM html_templates
        WHERE id = $1
    `
	if err := r.db.GetContext(ctx, &meta, query, id); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, fmt.Errorf("%s: %w", op, ErrNoTemplate)
		}
		return nil, fmt.Errorf("%s: %w", op, err)
	}
	return &meta, nil
}

func (r *htmlTemplateRepository) GetMetadataByName(ctx context.Context, name string) (*domain.HTMLTemplateMetadata, error) {
	op := "HTMLTemplateRepository.GetMetadataByName"
	var meta domain.HTMLTemplateMetadata

	query := `
        SELECT id, name, created_at, updated_at
        FROM html_templates
        WHERE name = $1
    `
	if err := r.db.GetContext(ctx, &meta, query, name); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, fmt.Errorf("%s: %w", op, ErrNoTemplate)
		}
		return nil, fmt.Errorf("%s: %w", op, err)
	}
	return &meta, nil
}

func (r *htmlTemplateRepository) List(ctx context.Context, limit, offset int) ([]domain.HTMLTemplateMetadata, error) {
	op := "HTMLTemplateRepository.List"
	templates := make([]domain.HTMLTemplateMetadata, 0)
	query := `
	SELECT id, name, created_at, updated_at
	FROM html_templates
	ORDER BY created_at DESC
	LIMIT $1 OFFSET $2
	`
	err := r.db.SelectContext(ctx, &templates, query, limit, offset)
	if err != nil {
		return nil, fmt.Errorf("%s: %w", op, err)
	}
	return templates, nil
}

func (r *htmlTemplateRepository) Create(ctx context.Context, template *domain.HTMLTemplate) error {
	op := "HTMLTemplateRepository.Create"
	query := `
	INSERT INTO html_templates (id, name, content, created_at, updated_at)
	VALUES (:id, :name, :content, :created_at, :updated_at)
	`
	_, err := r.db.NamedExecContext(ctx, query, template)
	if err != nil {
		return fmt.Errorf("%s: %w", op, err)
	}
	return nil
}

func (r *htmlTemplateRepository) Update(ctx context.Context, template *domain.HTMLTemplate) error {
	op := "HTMLTemplateRepository.Update"
	query := `
	UPDATE html_templates
	SET name = :name, content = :content, updated_at = :updated_at
	WHERE id = :id
	`
	_, err := r.db.NamedExecContext(ctx, query, template)
	if err != nil {
		return fmt.Errorf("%s: %w", op, err)
	}
	return nil
}

func (r *htmlTemplateRepository) Delete(ctx context.Context, id string) error {
	op := "HTMLTemplateRepository.Delete"
	query := `
	DELETE FROM html_templates
	WHERE id = $1
	`
	_, err := r.db.ExecContext(ctx, query, id)
	if err != nil {
		return fmt.Errorf("%s: %w", op, err)
	}
	return nil
}
